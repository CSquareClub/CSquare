...
async function ensureCusocOtpTable() {
  if (otpTableReady) return;

  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS cusoc_email_otps (
...
export async function getCusocOtpSentAt(uid: string): Promise<Date | null> {
  await ensureCusocOtpTable();

  const rows = await prisma.$queryRawUnsafe<Array<{ sent_at: Date | string }>>(
    `SELECT sent_at FROM cusoc_email_otps WHERE LOWER(uid) = LOWER($1::text) LIMIT 1;`,
    uid
  );
...
export async function upsertCusocOtp(params: {
  uid: string;
  collegeEmail: string;
  otpHash: string;
  expiresAt: Date;
  ...
  await prisma.$executeRawUnsafe(
    `INSERT INTO cusoc_email_otps (uid, college_email, otp_hash, expires_at, sent_at, attempt_count, verified, verified_at, used, updated_at)
...
export async function verifyCusocOtp(params: {
  uid: string;
  otpHash: string;
  maxAttempts?: number;
...
export async function isCusocOtpVerified(uid: string): Promise<boolean> {
  await ensureCusocOtpTable();
...
export async function markCusocOtpUsed(uid: string): Promise<void> {
  await ensureCusocOtpTable();
